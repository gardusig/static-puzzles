...
export default function NumeroxLetters() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div><h2 className="text-lg font-semibold text-gray-900">Numerox Letters</h2><p className="text-sm text-gray-400">Letter-based logic puzzle</p></div>
            ...
            <div className="inline-block border-2 border-gray-800 rounded">
              {grid.map((row, r) => (
                <div key={r} className="flex">{row.map((cell, c) => (
                  <input key={c} type="text" maxLength={1} value={cell} onChange={e => handleChange(r, c, e.target.value)}
                    className={`w-9 h-9 text-center border border-gray-200 text-sm uppercase ${initial[r]?.[c] !== '' ? 'bg-gray-50 font-semibold text-gray-700' : 'bg-white text-gray-900'} focus:outline-none focus:bg-indigo-50`} disabled={solved} />
                ))}</div>
              ))}
            </div>
