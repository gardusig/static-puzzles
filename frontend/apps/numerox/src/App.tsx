...
export default function Numerox() {
  ...
  const handleChange = (r: number, c: number, val: string) => {
    ...
    if (!isNaN(v) && v >= 0 && v <= 9) {
      const next = grid.map(row => [...row]); next[r][c] = v; setGrid(next)
    }
  }
  ...
  const handleCheck = async () => {
    ...
    else setMessage('Not quite right — check your numbers')
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Numerox</h2>
          <p className="text-sm text-gray-400">Number placement puzzle</p>
        </div>
            ...
            <div className="inline-block border-2 border-gray-800 rounded">
              {grid.map((row, r) => (
                <div key={r} className="flex">
                  {row.map((cell, c) => (
                    <input key={c} type="text" maxLength={1} value={cell === 0 ? '' : cell}
                      onChange={e => handleChange(r, c, e.target.value)}
                      className={`w-9 h-9 text-center border border-gray-200 text-sm ${initial[r]?.[c] !== 0 ? 'bg-gray-50 font-semibold text-gray-700' : 'bg-white text-gray-900'} focus:outline-none focus:bg-indigo-50`}
                      disabled={solved} />
                  ))}
                </div>
              ))}
        ...
        <div className="w-56 shrink-0">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            ...
            {history.length > 0 && <>
              ...
              <div className="space-y-1.5">{history.slice(0, 5).map((h, i) => <div key={i} className="text-xs text-gray-500 truncate">{h.difficulty} — {new Date(h.solvedAt).toLocaleDateString()}</div>)}</div>
            </>}
          </div>
        </div>
      </div>
