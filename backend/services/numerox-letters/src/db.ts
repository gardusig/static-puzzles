...
const PUZZLES_TABLE = 'numerox-letters-puzzles'
const PROGRESS_TABLE = 'numerox-letters-progress'
...
export async function getPuzzle(id: string) {
  const res = await doc.send(new GetCommand({ TableName: PUZZLES_TABLE, Key: { id } })); return res.Item
}
export async function getRandomPuzzle(difficulty?: string) {
  const all: unknown[] = []; let lk: Record<string, unknown> | undefined
  do {
    const r = await doc.send(new QueryCommand({
      TableName: PUZZLES_TABLE, KeyConditionExpression: 'begins_with(id, :p)',
      ExpressionAttributeValues: { ':p': 'puzzle_' }, ExclusiveStartKey: lk,
    }))
    if (r.Items) all.push(...r.Items); lk = r.LastEvaluatedKey
  } while (lk)
...
export async function getProgress(userId: string, puzzleId: string) {
  const r = await doc.send(new GetCommand({ TableName: PROGRESS_TABLE, Key: { id: `${userId}#${puzzleId}` } })); return r.Item
}
