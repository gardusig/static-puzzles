...
export const TABLES = {
  puzzles: 'numerox-letters-puzzles',
  progress: 'numerox-letters-progress',
} as const
...
export async function putPuzzle(puzzle: { id: string; data: unknown; solution: unknown; difficulty: string }) {
  await doc.send(new PutCommand({ TableName: TABLES.puzzles, Item: { ...puzzle, createdAt: new Date().toISOString() } }))
}
...
export async function getRandomPuzzle(difficulty?: string) {
  ...
  let filtered = items
  if (difficulty) filtered = items.filter(p => p.difficulty === difficulty)
  if (filtered.length === 0) return null
  return filtered[Math.floor(Math.random() * filtered.length)]
}

export async function saveProgress(userId: string, puzzleId: string, state: unknown, solved: boolean) {
  await doc.send(new PutCommand({ TableName: TABLES.progress, Item: { id: `${userId}#${puzzleId}`, userId, puzzleId, state, solved, updatedAt: new Date().toISOString() } }))
}

export async function getProgress(userId: string, puzzleId: string) {
  const res = await doc.send(new GetCommand({ TableName: TABLES.progress, Key: { id: `${userId}#${puzzleId}` } }))
  return res.Item as Record<string, unknown> | undefined
}
