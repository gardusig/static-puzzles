...
import { generateNumeroxLetters } from './generator.js'
...
router.get('/puzzle', async (req: Request, res: Response) => {
  ...
  if (!puzzle) {
    const g = generateNumeroxLetters(difficulty)
  ...
  res.json({ id: puzzle.id, grid: puzzle.data, words: (puzzle as Record<string, unknown>).words || [], difficulty: puzzle.difficulty })
})
