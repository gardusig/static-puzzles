...
const PUZZLES_TABLE = 'numerox-puzzles'
const PROGRESS_TABLE = 'numerox-progress'
...
export async function ensureTables() {
  await ensure(PUZZLES_TABLE)
  await ensure(PROGRESS_TABLE)
}
...
export async function getProgress(userId: string, puzzleId: string) {
  const res = await doc.send(new GetCommand({ TableName: PROGRESS_TABLE, Key: { id: `${userId}#${puzzleId}` } }))
  return res.Item
}
