...
const PUZZLES_TABLE = 'nqueens-puzzles'
const PROGRESS_TABLE = 'nqueens-progress'

async function ensure(table: string) {
  try { await doc.send(new CreateTableCommand({ TableName: table, AttributeDefinitions: [{ AttributeName: 'id', AttributeType: 'S' }], KeySchema: [{ AttributeName: 'id', KeyType: 'HASH' }], BillingMode: 'PAY_PER_REQUEST' })); console.log(`created table: ${table}`) }
...
export async function putPuzzle(puzzle: { id: string; data: unknown; solution: unknown; difficulty: string }) { await doc.send(new PutCommand({ TableName: PUZZLES_TABLE, Item: { ...puzzle, createdAt: new Date().toISOString() } })) }
...
export async function getRandomPuzzle(difficulty?: string) {
  ...
  let f = all as Array<Record<string, unknown>>; if (difficulty) f = f.filter(p => p.difficulty === difficulty); return f.length ? f[Math.floor(Math.random() * f.length)] : null
}
export async function saveProgress(userId: string, puzzleId: string, state: unknown, solved: boolean) { await doc.send(new PutCommand({ TableName: PROGRESS_TABLE, Item: { id: `${userId}#${puzzleId}`, userId, puzzleId, state, solved, updatedAt: new Date().toISOString() } })) }
