...
router.get('/puzzle', async (req: Request, res: Response) => {
  ...
  if (!puzzle) {
    const g = generateCrossword(difficulty)
    const id = `puzzle_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`
    await putPuzzle({ id, data: { clues: g.clues, grid: g.grid }, solution: g.solution, difficulty })
    puzzle = { id, data: { clues: g.clues, grid: g.grid }, solution: g.solution, difficulty }
  }
  const d = puzzle.data as Record<string, unknown>
  res.json({ id: puzzle.id, grid: d.grid, clues: d.clues, difficulty: puzzle.difficulty })
})
